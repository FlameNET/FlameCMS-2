...
var Creation = Class.extend({
	...
	setup: function() {
		...
		this.form.bind('validate', { form: this }, function(e) {
			if (document.activeElement && $('#emailAddress').val() && document.activeElement.id != 'emailAddress' && document.activeElement.id != 'emailAddressConfirmation') {
				e.data.form.validateEmail();
			}
			if (document.activeElement && $('#parentContactEmail').val() && document.activeElement.id != 'parentContactEmail' && document.activeElement.id != 'parentContactEmailConfirmation') {
		...
		this.form.on('submit', { form: this }, function(evt) {
			var blank = false,
				badEmail = false,
				unchecked = false,
				errors = [],
			...
			if (evt.data.form.countryBlocked) {
				errors.push(FormMsg.selectCountry);
				badFields.push(evt.data.form.countrySelect, true);
			} else {
				for (var i = 0, len = fields.length; i < len; i++) {
					field = fields[i];
					if (!field.value) {
						if (!blank) {
							errors.push(FormMsg.fieldInvalid, FormMsg.fieldsMissing);
							blank = true;
						}
						badFields.push(field);
					} else if (field.id === 'emailAddress' && !evt.data.form.emailValidated) {
						errors.push(FormMsg.emailError1);
						badFields.push(field);
					} else if (field.id === 'emailAddressConfirmation' && !evt.data.form.emailConfirmationValidated) {
						errors.push(FormMsg.emailError2);
						badFields.push(field);
					}else if (field.id.indexOf('password') > -1 && !evt.data.form.passwordValidated) {
						errors.push(FormMsg.passwordError1);
						badFields.push(field);
				...
				evt.data.form.toggleErrorFlag(badFields, fields);
			}

			if (errors.length) {
				$('#content').prepend(evt.data.form.makeErrorBox(errors));
				$('#password-strength').slideUp(evt.data.form.slideUpDuration);
				$('#contact-info').slideUp(evt.data.form.slideUpDuration);
	...
	validateEmail: function() {
		var email1 = this.emailInput1[0].value,
		...
		this.emailConfirmationValidated = true;
		...
		if (email2 !== '' && results[1] === -1) {
			...
			this.emailConfirmationValidated = false;
		} else {
			this.emailMessage2.html(' ');
		}
		...
		if (this.emailValidated && this.emailConfirmationValidated) {
		...
		return this.emailValidated && this.emailConfirmationValidated;
	},
	 ...
	 */
	toggleErrorFlag: function(badFields, allFields) {
		var field;
		
		allFields.parents('span.input-right').removeClass('input-error');
		allFields.parents('span.input-text').removeClass('input-error');
		allFields.parents('span.input-right').siblings('span.input-left').removeClass('input-error');

		for (var i = 0, len = badFields.length; i < len; i++) {
			field = $(badFields[i]);
			if (field.parent('span.input-text').length) {
				field.parent('span.input-text').addClass('input-error');
		} else {
				field.parent('span.input-right').addClass('input-error');
		}
		}
		$(badFields).parents('span.input-right').siblings('span.input-left').addClass('input-error');
	},
